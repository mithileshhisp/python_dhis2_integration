...
def construct_sql_query_event_payload(org_unit_id, tei_data, Visitcode, BenVisitID, BeneficiaryRegID,
...
def push_dataValueSet_in_dhis2_xlsx(dataValueSet_payload, orgUnit, row_no):
    #print(f"dataValueSet_payload : {dataValueSet_payload}")
    ...
    if response.status_code == 200:
        ...
        print(f"DataValue created successfully. Row No : {row_no} . orgUnit : {orgUnit} . impCount : {impCount} . updateCount : {updateCount} . ignoreCount : {ignoreCount} . description : {description}")
        logging.info(f"DataValue created successfully. Row No : {row_no} . orgUnit : {orgUnit} .impCount : {impCount} . updateCount : {updateCount} . ignoreCount: {ignoreCount} . description : {description}")
        
    else:
        print(f"Failed to create dataValueSet. Error: {response.text}")
        logging.error(f"Failed to dataValueSet events . Row No : {row_no} . orgUnit : {orgUnit} . conflictsDetails : {conflictsDetails} . Status code: {response.status_code} . error details: {response.json()} .Error: {response.text}")
