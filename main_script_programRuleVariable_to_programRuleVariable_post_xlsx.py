...
def get_programRuleVariables_details(session_get, programRuleVariable_uid):
    ...
    #print( f"program_rule_variable_get_url . { program_rule_variable_get_url }" )
...
def push_programrule_variable_in_dhis2(session_post, programRuleVariable_payload, programRuleVariable_uid, row ):
    ...
    try:
        ...
        print(f" Failed to create programRuleVariable_uid. {programRuleVariable_uid} Error: {response.text}")
        logging.error(f"Failed to create programRuleVariable_uid : {programRuleVariable_uid} . row : {row} . Status code: {response.status_code} . error details: {response.json()} .Error: {response.text}")
