...
dataValueSet_endPoint = f"{dhis2_api_url}dataValueSets"
...
def construct_sql_query_event_payload(org_unit_id, tei_data, Visitcode, BenVisitID, BeneficiaryRegID, VisitNo, CreatedDate, vanid, AgeOnVisit, VisitReason, VisitCategory, PregnancyStatus, Referal_Visitcode, referredToInstituteName, referralreason, ProvisionalDiagnosis, phyanthropometry_bmi, nurse_rbs, Weight_Kg, Height_cm, WaistCircumference_cm, Temperature, SystolicBP_1stReading, DiastolicBP_1stReading, ComorbidCondition_Fulltext, Comobrid_Diabetes, Comobrid_Hypertension, Comobrid_Asthma, Comobrid_Epilepsy, Comobrid_Heart_Disease, Comobrid_Kidney_Disease, Comobrid_Sickle_cell, Labtest_Prescribed, Drug_Prescribedcode, Drug_Dispensecode, diagnosisprovided_full_text, DiagnosisProvided1, NCD_Condition, ncd_condition_full_text, PulseRate, spo2, feto_Visitcode, TestName, GenericDrugName, CreatedBy, ChiefComplaint, ClinicalObservation, OtherSymptoms, RespiratoryRate, NCDCareType, NCD_Complication, TobaccoUseStatus, AlcoholIntakeStatus, Servicepoint, SpouseName, FatherName, occupation, MaritalStatus, LiteracyStatus, Education, IncomeStatus, MonthlyFamilyIncome, Community, Religion, State, Block, Village, RCHID, Labtest_Done_Visitcode, Total_lab_test, Random_Blood_Sugar, Hemoglobin, UrineAlbumin, UrineSugar, Urine_Pregnancy_test, Malaria, ECG, HbA1c, Liver_Function_Test, Liverfunctiontest_Direct_Bilirubin, Liverfunctiontest_SGPT, Liverfunctiontest_SGOT, Liverfunctiontest_Total_Serum_Bilirubin, Liverfunctiontest_Blood_Urea, Liverfunctiontest_Serum_Creatinine, Renal_function_test, Renalfunctiontest_Serum_Creatinine, Renalfunctiontest_Blood_Urea, Lipid_profile, Lipidprofile_Serum_Total_Cholesterol, Lipidprofile_Serum_HDL, Lipidprofile_LDL, Lipidprofile_Serum_Triglycerides, Post_Lunch_Blood_Sugar, Fasting_Blood_Sugar, Complete_Blood_Picture, CBP_ESR, CBP_Monocytes, CBP_Hemoglobin, CBP_RBC, CBP_Lymphocytes, CBP_Neutrophils, CBP_Basophils, CBP_Eosinophils, CBP_MCV, CBP_MCH, CBP_MCHC, CBP_Hematocrit, CBP_Platelet_Count, CBP_Total_Leucocyte_Count, Widal_Test, Sputum_AFB_Test, Sickle_Cell_Disease_Test, HBsAg, Complete_Urine_Examination, Complete_Urine_Examination_Total_Leucocyte_Count, Complete_Urine_Examination_Urine_for_Nitrite, Chikungunya, Hb_Electrophoresis, Dengue_NS1_Antigen, Dengue_Antibody_Test, HIV1_HIV2_RDT, Visual_Acuity_Test, Blood_Group, VDRL_Test, Syphilis, Serum_Uric_Acid, Serum_Total_Cholesterol, Hepatitis_B, Hepatitis_C, ESR, RBS_Status, Drug_Visitcode, Total_drug, Drug_1, Drug1_Quantity, Drug_2, Drug2_Quantity, Drug_3, Drug3_Quantity, Drug_4, Drug4_Quantity, Drug_5, Drug5_Quantity, Drug_6, Drug6_Quantity, Drug_7, Drug7_Quantity, Drug_8, Drug8_Quantity, Drug_9, Drug9_Quantity, Drug_10, Drug10_Quantity, Disp_Visitcode, dispensed_qty ):
    
    #orgUnit = tei_data["orgUnit"]
    tei_uid = tei_data["trackedEntityInstance"]
    eventDate = CreatedDate.strftime("%Y-%m-%d")

    existing_event = check_existing_event(org_unit_id, Visitcode)

    #print(f"event_payload: {existing_event[0]['orgUnitName']}" )
    if existing_event:
        print(f"Event already exists for EventId {existing_event[0]['event']}. Visitcode {Visitcode}. BeneficiaryRegID {BeneficiaryRegID}. orgUnit Id {org_unit_id}. orgUnitName {existing_event[0]['orgUnitName']}. Skipping.")
        logging.info(f"Event already exists for EventId {existing_event[0]['event']}. Visitcode {Visitcode}. BeneficiaryRegID {BeneficiaryRegID}. orgUnit Id {org_unit_id}. orgUnitName {existing_event[0]['orgUnitName']}. Skipping.")
        return
    ...
    event_payload = {
        ...
        "dataValues": [
            ...
            {"dataElement": "fHKrVsWMbib", "value": assign_value_if_NaN(TobaccoUseStatus)},
            {"dataElement": "vFEaiiA2YUw", "value": assign_value_if_NaN(AlcoholIntakeStatus)},
            {"dataElement": "zfMNtSNz63s", "value": assign_value_if_NaN(Servicepoint)},
            ...
            {"dataElement": "n3NEYRyQ18n", "value": assign_value_if_NaN(MaritalStatus)},
            {"dataElement": "oiAaI2U3yFd", "value": assign_value_if_NaN(LiteracyStatus)},
            {"dataElement": "Hj308x3Mx4X", "value": assign_value_if_NaN(Education)},
            {"dataElement": "zIrM7R9RxFS", "value": assign_value_if_NaN(IncomeStatus)},
            {"dataElement": "jJDtZOoj3b1", "value": assign_value_if_NaN(MonthlyFamilyIncome)},
            {"dataElement": "WlrNyMC0lgV", "value": assign_value_if_NaN(Community)},
            {"dataElement": "d2NvN6mkZPk", "value": assign_value_if_NaN(Religion)},
            {"dataElement": "apifVBtGLer", "value": assign_value_if_NaN(State)},
            {"dataElement": "jlpOifpoA3F", "value": assign_value_if_NaN(Block)},
            {"dataElement": "OPO6rgJdl0S", "value": assign_value_if_NaN(Village)},
            {"dataElement": "o3aGx9XBFGg", "value": assign_value_if_NaN(Drug_1)},
            {"dataElement": "fpuiGZr4IdN", "value": assign_value_if_NaN(Drug1_Quantity)},
            {"dataElement": "vqbPOXVQu4U", "value": assign_value_if_NaN(Drug_2)},
            {"dataElement": "fFhbbH4dnnk", "value": assign_value_if_NaN(Drug2_Quantity)},
            {"dataElement": "LmV47pW2H6D", "value": assign_value_if_NaN(Drug_3)},
            {"dataElement": "R6m8Olg37Kk", "value": assign_value_if_NaN(Drug3_Quantity)},
            {"dataElement": "moPjVcpyE0H", "value": assign_value_if_NaN(Drug_4)},
            {"dataElement": "MN3AA8d9CN3", "value": assign_value_if_NaN(Drug4_Quantity)},
            {"dataElement": "Zhjqktl9uih", "value": assign_value_if_NaN(Drug_5)},
            {"dataElement": "QtaFUVASUmO", "value": assign_value_if_NaN(Drug5_Quantity)},
            {"dataElement": "p42ZUgfimrC", "value": assign_value_if_NaN(Drug_6)},
            {"dataElement": "oD5AzRmoB0L", "value": assign_value_if_NaN(Drug6_Quantity)},
            {"dataElement": "LxhvRx9OeHh", "value": assign_value_if_NaN(Drug_7)},
            {"dataElement": "NI819aS76L6", "value": assign_value_if_NaN(Drug7_Quantity)},
            {"dataElement": "ui7a8HU27gL", "value": assign_value_if_NaN(Drug_8)},
            {"dataElement": "Iyst9wI7kWy", "value": assign_value_if_NaN(Drug8_Quantity)},
            {"dataElement": "zzzzBHMQDim", "value": assign_value_if_NaN(Drug_9)},
            {"dataElement": "kyvYAbxm4pr", "value": assign_value_if_NaN(Drug9_Quantity)},
            {"dataElement": "LBJsC2Rw41v", "value": assign_value_if_NaN(Drug_10)},
            {"dataElement": "FvuBvpjnGmG", "value": assign_value_if_NaN(Drug10_Quantity)},
            {"dataElement": "Uq3Y3b6idEt", "value": assign_value_if_NaN(dispensed_qty)},
            {"dataElement": "IRBUYDrIlzb", "value": assign_value_if_NaN(Drug_Visitcode)},
            {"dataElement": "IRBUYDrIlzb", "value": assign_value_if_NaN(Disp_Visitcode)},
            {"dataElement": "hx87erleZAB", "value": assign_value_if_NaN(CreatedBy)},
            {"dataElement": "xksMytLmsog", "value": assign_value_if_NaN(ChiefComplaint)},
            {"dataElement": "A2HNLbaidOv", "value": assign_value_if_NaN(ClinicalObservation)},
            {"dataElement": "YtayheptZL1", "value": assign_value_if_NaN(OtherSymptoms)},
            {"dataElement": "Vg4dGdpNkae", "value": assign_value_if_NaN(RespiratoryRate)},
            {"dataElement": "mggkRjss6BN", "value": assign_value_if_NaN(NCDCareType)},
            {"dataElement": "mBYW3pZhEl4", "value": assign_value_if_NaN(NCD_Complication)},
            {"dataElement": "ziHAA9w0OMI", "value": assign_value_if_NaN(occupation)},
            {"dataElement": "LyKVDsTWOVv", "value": assign_value_if_NaN(RCHID)},
            {"dataElement": "kJT7phq0KxA", "value": assign_value_if_NaN(SpouseName)},
            {"dataElement": "R0wTH2GLS1U", "value": assign_value_if_NaN(FatherName)},

            
            {"dataElement": "fVFoB96fAK1", "value": assign_value_if_NaN("STFC")}
        ]
...
def construct_xlsx_event_payload(trackedEntityInstance, orgUnit, Visitcode, BenVisitID, BeneficiaryRegID,
    ...
    event_payload = {
        ...
        "dataValues": [
            ...
            {"dataElement": "hGZ4pC3EY6c", "value": assign_value_if_NaN(ESR)},
            {"dataElement": "nCMJQKramRa", "value": assign_value_if_NaN(RBS_Status)},
            {"dataElement": "n3NEYRyQ18n", "value": assign_value_if_not_null(MaritalStatus)},
            {"dataElement": "oiAaI2U3yFd", "value": assign_value_if_not_null(LiteracyStatus)},
            {"dataElement": "Hj308x3Mx4X", "value": assign_value_if_not_null(Education)},
            {"dataElement": "zIrM7R9RxFS", "value": assign_value_if_not_null(IncomeStatus)},
            {"dataElement": "jJDtZOoj3b1", "value": assign_value_if_not_null(MonthlyFamilyIncome)},
            {"dataElement": "WlrNyMC0lgV", "value": assign_value_if_not_null(Community)},
            {"dataElement": "d2NvN6mkZPk", "value": assign_value_if_not_null(Religion)},
            {"dataElement": "apifVBtGLer", "value": assign_value_if_not_null(State)},
            {"dataElement": "jlpOifpoA3F", "value": assign_value_if_not_null(Block)},
            {"dataElement": "OPO6rgJdl0S", "value": assign_value_if_not_null(Village)},
            {"dataElement": "o3aGx9XBFGg", "value": assign_value_if_not_null(Drug_1)},
            {"dataElement": "fpuiGZr4IdN", "value": assign_value_if_not_null(Drug1_Quantity)},
            {"dataElement": "vqbPOXVQu4U", "value": assign_value_if_not_null(Drug_2)},
            {"dataElement": "fFhbbH4dnnk", "value": assign_value_if_not_null(Drug2_Quantity)},
            {"dataElement": "LmV47pW2H6D", "value": assign_value_if_not_null(Drug_3)},
            {"dataElement": "R6m8Olg37Kk", "value": assign_value_if_not_null(Drug3_Quantity)},
            {"dataElement": "moPjVcpyE0H", "value": assign_value_if_not_null(Drug_4)},
            {"dataElement": "MN3AA8d9CN3", "value": assign_value_if_not_null(Drug4_Quantity)},
            {"dataElement": "Zhjqktl9uih", "value": assign_value_if_not_null(Drug_5)},
            {"dataElement": "QtaFUVASUmO", "value": assign_value_if_not_null(Drug5_Quantity)},
            {"dataElement": "p42ZUgfimrC", "value": assign_value_if_not_null(Drug_6)},
            {"dataElement": "oD5AzRmoB0L", "value": assign_value_if_not_null(Drug6_Quantity)},
            {"dataElement": "LxhvRx9OeHh", "value": assign_value_if_not_null(Drug_7)},
            {"dataElement": "NI819aS76L6", "value": assign_value_if_not_null(Drug7_Quantity)},
            {"dataElement": "ui7a8HU27gL", "value": assign_value_if_not_null(Drug_8)},
            {"dataElement": "Iyst9wI7kWy", "value": assign_value_if_not_null(Drug8_Quantity)},
            {"dataElement": "zzzzBHMQDim", "value": assign_value_if_not_null(Drug_9)},
            {"dataElement": "kyvYAbxm4pr", "value": assign_value_if_not_null(Drug9_Quantity)},
            {"dataElement": "LBJsC2Rw41v", "value": assign_value_if_not_null(Drug_10)},
            {"dataElement": "FvuBvpjnGmG", "value": assign_value_if_not_null(Drug10_Quantity)},
            {"dataElement": "Uq3Y3b6idEt", "value": assign_value_if_not_null(dispensed_qty)},
            {"dataElement": "IRBUYDrIlzb", "value": assign_value_if_not_null(Drug_Visitcode)},
            {"dataElement": "IRBUYDrIlzb", "value": assign_value_if_not_null(Disp_Visitcode)},
            {"dataElement": "hx87erleZAB", "value": assign_value_if_not_null(CreatedBy)},
            {"dataElement": "xksMytLmsog", "value": assign_value_if_not_null(ChiefComplaint)},
            {"dataElement": "A2HNLbaidOv", "value": assign_value_if_not_null(ClinicalObservation)},
            {"dataElement": "YtayheptZL1", "value": assign_value_if_not_null(OtherSymptoms)},
            {"dataElement": "Vg4dGdpNkae", "value": assign_value_if_not_null(RespiratoryRate)},
            {"dataElement": "mggkRjss6BN", "value": assign_value_if_not_null(NCDCareType)},
            {"dataElement": "mBYW3pZhEl4", "value": assign_value_if_not_null(NCD_Complication)},
            {"dataElement": "ziHAA9w0OMI", "value": assign_value_if_not_null(occupation)},
            {"dataElement": "LyKVDsTWOVv", "value": assign_value_if_not_null(RCHID)},
            {"dataElement": "kJT7phq0KxA", "value": assign_value_if_not_null(SpouseName)},
            {"dataElement": "R0wTH2GLS1U", "value": assign_value_if_not_null(FatherName)},
...
def construct_xlsx_dataValueSet_payload(dataValues, orgUnit):
...
def push_dataValueSet_in_dhis2_xlsx(session_get_post, dataValueSet_payload, tempIsoPeriod, row_no):
    #print(f"dataValueSet_payload : {dataValueSet_payload}")
    #json_object = json.loads(dataValueSet_payload)
    '''
    response = requests.post(
        f"{dhis2_api_url_ippf}dataValueSets",
        json=dataValueSet_payload,
        auth=(dhis2_username_ippf, dhis2_password_ippf)
    )
    '''
    #, verify=False
    #temp_dataValueSet_payload = np.int64(dataValueSet_payload)
    #temp_temp_dataValueSet_payload = int(temp_dataValueSet_payload)
    '''
    response = requests.post(
        dataValueSet_endPoint,
        data=json.dumps(dataValueSet_payload),
        #data=dataValueSet_payload,
        headers={
            "Content-Type": "application/json",
            "Authorization": f"Basic {base64.b64encode(f'{dhis2_username}:{dhis2_password}'.encode()).decode()}"
        }
    ...
    if response.status_code == 200:
        ...
        print(f"DataValue created successfully. Row No : {row_no} . tempIsoPeriod : {tempIsoPeriod} . impCount : {impCount} . updateCount : {updateCount} . ignoreCount : {ignoreCount} . description : {description}")
        logging.info(f"DataValue created successfully. Row No : {row_no} . tempIsoPeriod : {tempIsoPeriod} .impCount : {impCount} . updateCount : {updateCount} . ignoreCount: {ignoreCount} . description : {description}")
        
    else:
        print(f"Failed to create dataValueSet. Error: {response.text}")
        logging.error(f"Failed to dataValueSet events . Row No : {row_no} . tempIsoPeriod : {tempIsoPeriod} . conflictsDetails : {conflictsDetails} . Status code: {response.status_code} . error details: {response.json()} .Error: {response.text}")
...
def update_eventDataValue_in_dhis2_xlsx(updateEventDataValue, eventUID, dataElementUid, row_no):
    event_search_url = f"{dhis2_api_url}events/{eventUID}/{dataElementUid}"
    response = requests.put(
        event_search_url,
        json=updateEventDataValue,
        auth=(dhis2_username, dhis2_password)
    )
