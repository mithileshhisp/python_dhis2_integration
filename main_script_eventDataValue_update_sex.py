...
from constants import LOG_FILE_EVENT_DATA_VALUE_UPDATE_SEX
...
logging.basicConfig(filename=LOG_FILE_EVENT_DATA_VALUE_UPDATE_SEX, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
print( f" update evenDataValue SEX start . { current_time_start }" )
logging.info(f" update eventDataValue SEX start . { current_time_start }")
...
def get_sqlview_data():
    ...
    sql_view_url = f"{DHIS2_API_URL}sqlViews/wGdLwfDzjyG/data?paging=false"
...
print( f" update eventDataValue SEX finished . { current_time_end }" )
logging.info(f" update eventDataValue SEX finished . { current_time_end }")
