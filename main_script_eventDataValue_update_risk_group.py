...
from constants import LOG_FILE_EVENT_DATA_VALUE_UPDATE_RISK_GROUP
...
logging.basicConfig(filename=LOG_FILE_EVENT_DATA_VALUE_UPDATE_RISK_GROUP, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
print( f" update evenDataValue Risk group start . { current_time_start }" )
logging.info(f" update eventDataValue Risk group start . { current_time_start }")
...
def get_sqlview_data():
    ...
    sql_view_url = f"{DHIS2_API_URL}sqlViews/nQT7OA90MOq/data?paging=false"
...
print( f" update eventDataValue Risk group finished . { current_time_end }" )
logging.info(f" update eventDataValue Risk group finished . { current_time_end }")
