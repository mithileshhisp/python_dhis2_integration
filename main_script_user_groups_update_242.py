...
from constants import LOG_FILE_USERS_GROUP_UPDATE
...
log_filename = LOG_FILE_USERS_GROUP_UPDATE
...
print( f"user Group Updation start at. { current_time_start }" )
logging.info(f"user Group Updation start at. { current_time_start }")
...
df = pd.read_excel("userGroupsUpdate.xlsx", sheet_name="userGroupsUpdate")
# Convert NaN → None
...
for i, row in enumerate(records, start=1):
    ...
    if len(data.get("users", [])) > 0:
        ...
        if row.get("userGroups"):
            updateUserGroups["userGroups"] = [{"id": x.strip()} for x in str(row["userGroups"]).split(",")]
        '''
        ...
        put_response = session_post.put(
            put_url,
            json=updateUserGroups
        )

        if put_response.status_code in [200, 201, 204]:
            print(f"Row {i}: User Group Updated success → {put_response.json()}")
            logging.info(f"Row {i}: User Group Updated success : {put_response.json()}")
...
print( f"user Group Updation end at. { current_time_end }" )
logging.info(f"user Group Updation end at. { current_time_end }")