# python_dhis2_integration
Python based data integration from dhis2 and other application and add in schedular ( cron job )

# hiv tracker push missing event datavalue through schedular

This Python script facilitates the push of event data value and update events in DHIS2.

## Prerequisites


##install python
sudo apt-get install python
##install pip
sudo apt-get install python-pip

sudo apt-get install python3-pip

pip install mysql-connector-python

pip install mysql-connector-python requests pandaspip install openpyxl

-- ps -aux 
python -V

## Usage

Run the script using the following command:

```bash
python main_script_eventDataValue_update_age.py

### Author

- **mithileshhisp** - [GitHub Profile](https://github.com/mithileshhisp/)
