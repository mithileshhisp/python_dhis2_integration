...
from constants import LOG_FILE_EVENT_DATA_VALUE_UPDATE_CLIENT_OF_FSW_TYPE
...
logging.basicConfig(filename=LOG_FILE_EVENT_DATA_VALUE_UPDATE_CLIENT_OF_FSW_TYPE, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
print( f" update evenDataValue Client of FSW type start . { current_time_start }" )
logging.info(f" update eventDataValue Client of FSW type start . { current_time_start }")
...
def get_sqlview_data():
    ...
    sql_view_url = f"{DHIS2_API_URL}sqlViews/CTAfO6mvhTl/data?paging=false"
...
print( f" update eventDataValue Client of FSW type finished . { current_time_end }" )
logging.info(f" update eventDataValue Client of FSW type finished . { current_time_end }")
