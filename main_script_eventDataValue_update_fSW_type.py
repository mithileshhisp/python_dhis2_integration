...
from constants import LOG_FILE_EVENT_DATA_VALUE_UPDATE_FSW_TYPE
...
logging.basicConfig(filename=LOG_FILE_EVENT_DATA_VALUE_UPDATE_FSW_TYPE, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
print( f" update evenDataValue FSW type start . { current_time_start }" )
logging.info(f" update eventDataValue FSW type start . { current_time_start }")
...
def get_sqlview_data():
    ...
    sql_view_url = f"{DHIS2_API_URL}sqlViews/GWdWZBnvv0G/data?paging=false"
...
print( f" update eventDataValue FSW type finished . { current_time_end }" )
logging.info(f" update eventDataValue FSW type finished . { current_time_end }")
