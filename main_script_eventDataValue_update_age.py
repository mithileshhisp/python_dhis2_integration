...
from constants import LOG_FILE_EVENT_DATA_VALUE_UPDATE_AGE
...
logging.basicConfig(filename=LOG_FILE_EVENT_DATA_VALUE_UPDATE_AGE, level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s")
...
print( f" update evenDataValue Age On Visit start . { current_time_start }" )
logging.info(f" update eventDataValue Age On Visit start . { current_time_start }")
...
def get_sqlview_data():
    ...
    sql_view_url = f"{DHIS2_API_URL}sqlViews/P8cFNnfn9UP/data?paging=false"
...
print( f" update eventDataValue Age On Visit finished . { current_time_end }" )
logging.info(f" update eventDataValue Age On Visit finished . { current_time_end }")
