...
from constants import LOG_FILE_USERS_DISABLED_UPDATE
...
log_filename = LOG_FILE_USERS_DISABLED_UPDATE
...
print( f"user disabled Updation start at. { current_time_start }" )
logging.info(f"user disabled Updation start at. { current_time_start }")
...
df = pd.read_excel("usersDisabled.xlsx", sheet_name="usersDisabled")
# Convert NaN → None
...
for i, row in enumerate(records, start=1):
    ...
    if len(data.get("users", [])) > 0:
        ...
        updateUsersDisabled = user

        #This handles: None,NaN,empty string "", spaces " "
        if pd.notna(row.get("disabled")) and str(row.get("disabled")).strip():
            updateUsersDisabled["disabled"] = row.get("disabled")

        put_url = f"{DHIS2_API_POST_URL}users/{user_id}"

        put_response = session_post.put(
            put_url,
            json=updateUsersDisabled
        )

        if put_response.status_code in [200, 201, 204]:
            print(f"Row {i}: User disabled success → {put_response.json()}")
            logging.info(f"Row {i}: User disabled success : {put_response.json()}")
...
print( f"user disabled Updation end at. { current_time_end }" )
logging.info(f"user disabled Updation end at. { current_time_end }")